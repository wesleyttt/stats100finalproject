...
per_yr1 = c()
per_yr2 = c()
prediction_df <- data.frame(matrix(ncol = 5, nrow = 0))
colnames(prediction_df) <-c("name", "per_yr1", "per_yr2", "pts_yr1", "pts_yr2")
for (x in 2:1054){
  player_df = filter(with_prime, name == player_names[x,1]) 
  if (dim(player_df)[1] > 2){
    player_df = player_df[order(player_df$Year),]
    new_row = c(player_df[1,1],player_df[1,7],player_df[2,7],player_df[1,13], player_df[2,13])
